...
test('sync supports the built-in Zed adapter', () => {
  const dir = tempProject()
  writeFile(path.join(dir, 'pluribus.md'), requiredSections)

  const result = spawnSync(
    process.execPath,
    [path.join(repoRoot, 'bin', 'pluribus.js'), 'sync', '--tools', 'zed'],
    { cwd: dir, encoding: 'utf8' }
  )
  ...
  const rules = fs.readFileSync(path.join(dir, '.rules'), 'utf8')
  assert.match(rules, /# Zed AI Rules — generated by Pluribus/)
  assert.match(rules, /## Conventions\nShared conventions/)
  assert.match(rules, /## Constraints\nShared constraints/)
})
