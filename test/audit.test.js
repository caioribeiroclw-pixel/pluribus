...
test('audit can include a portability fidelity report in JSON output', () => {
  ...
  assert.equal(cursorTarget.duplicateLoadEvidence.duplicateRisk, 'unknown')
  assert.equal(cursorTarget.duplicateLoadEvidence.selectedLoad.path, '.cursorrules')
  assert.match(cursorTarget.duplicateLoadEvidence.contentIdentity, /^sha256:/)
  assert.deepEqual(cursorTarget.semanticDifference, ['section-loss', 'project-wide-only', 'no-path-scope-evidence', 'runtime-load-dedupe-not-proven', 'duplicate-load-selection-not-proven'])
  ...
  assert.ok(payload.fidelityReport.warnings.some((warning) => warning.code === 'duplicate-load-selection-not-proven'))
...
`)
  ...
  assert.equal(bobTarget.duplicateLoadEvidence.selectedLoad.path, '.bob/rules/pluribus.md')
  assert.match(bobTarget.duplicateLoadEvidence.contentIdentity, /^sha256:/)
  assert.equal(bobTarget.duplicateLoadEvidence.suppressedLoads.length, 0)
  assert.ok(bobTarget.semanticDifference.includes('duplicate-load-selection-not-proven'))
...
test('audit JSON schema is packaged and matches the emitted top-level contract', () => {
  ...
  assert.ok(schema.$defs.fidelityTarget.required.includes('duplicateLoadEvidence'))
  assert.deepEqual(schema.$defs.loadEvidence.required.slice(0, 4), ['loadedBy', 'effectiveSource', 'deliveryPath', 'deliveryMechanism'])
  assert.ok(schema.$defs.duplicateLoadEvidence.required.includes('selectedLoad'))
