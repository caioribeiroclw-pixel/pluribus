...
test('public handoff links resolve and disclose the project identity', () => {
  ...
  const englishJourneyPath = 'docs/CAIO-RIBEIRO-JOURNEY.md'
  const englishJourneyPagePath = 'docs/caio-ribeiro-journey.html'

  for (const artifactPath of [diaryPath, memoPath, journeyPath, journeyPagePath, englishJourneyPath, englishJourneyPagePath]) {
  ...
  assert.match(readme, new RegExp(`\\(${englishJourneyPath}\\)`))
  assert.match(readme, /caio-ribeiro-jornada\.html/)
  assert.match(readme, /caio-ribeiro-journey\.html/)
  assert.match(siteIndex, /href="caio-ribeiro-jornada\.html"/)
  assert.match(siteIndex, /href="caio-ribeiro-journey\.html"/)
