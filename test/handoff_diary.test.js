...
test('handoff diary sync preserves the source byte-for-byte', () => {
  ...
  const previousOutput = fs.existsSync(outputPath) ? fs.readFileSync(outputPath) : null
  ...
  try {
    ...
    if (previousOutput === null) fs.rmSync(outputPath, { force: true })
    else fs.writeFileSync(outputPath, previousOutput)
  }
})
