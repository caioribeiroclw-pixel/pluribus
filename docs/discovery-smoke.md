...
The JSON output is meant to be pasted into activity logs without exposing secrets or local project context. Download counts can include local smoke/release installs, so treat them as directional unless there is external corroboration such as stars, issues, replies, or third-party mentions.
