...
Start read-only so you can see the template before writing a file. `init --dry-run` is prepared for `pluribus-context@0.3.1`; until that patch is published, use the GitHub tag install command for the preview:
