...
`pluribus audit --strict` is read-only: it fails when a generated file is missing or drifted, but it does not rewrite anything in CI. The basic audit command is published in `pluribus-context@0.3.0`; the `--github-annotations`, `--json`, `--output`, and `--ci` flags below are available in the GitHub release tag `v0.3.2` while npm latest waits for the `0.3.2` publish. Until that patch is published, pin CI to `pluribus-context@0.3.0` for the strict text check, or test the new flags from the immutable GitHub tag with `--package github:caioribeiroclw-pixel/pluribus#v0.3.2`.
  ...
  audit:
    ...
    steps:
      ...
      - name: Audit AI context drift
        ...
        run: npx --yes --package github:caioribeiroclw-pixel/pluribus#v0.3.2 pluribus audit --ci
```
...
```yaml
      - name: Audit AI context drift as JSON
        # Tagged v0.3.2 path until pluribus-context@0.3.2 is published to npm.
        run: npx --yes --package github:caioribeiroclw-pixel/pluribus#v0.3.2 pluribus audit --ci --json --output pluribus-audit.json
...
# In GitHub Actions with the tagged 0.3.2 CI flags, use:
